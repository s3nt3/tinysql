...
func DecodeRecordKey(key kv.Key) (tableID int64, handle int64, err error) {
	...
	if len(key) != RecordRowKeyLen {
		return tableID, handle, errors.New("decode record key failed, invalid key length")
	}

	_, tableID, err = codec.DecodeInt(key[tablePrefixLength:])
	if err != nil {
		return tableID, handle, errors.New("decode record key failed, invalid tableID")
	}

	_, handle, err = codec.DecodeInt(key[prefixLen:])
	if err != nil {
		return tableID, handle, errors.New("decode record key failed, invalid handle")
	}
...
func DecodeIndexKeyPrefix(key kv.Key) (tableID int64, indexID int64, indexValues []byte, err error) {
	...
	if len(key) <= prefixLen {
		return tableID, indexID, nil, errors.New("decode index key failed, invalid key length")
	}

	_, tableID, err = codec.DecodeInt(key[tablePrefixLength:])
	if err != nil {
		return tableID, indexID, nil, errors.New("decode index key failed, invalid tableID")
	}

	indexValues, indexID, err = codec.DecodeInt(key[prefixLen:])
	if err != nil {
		return tableID, indexID, nil, errors.New("decode index key failed, invalid indexID")
	}
